...
if [ -f "$CONFIG_FILE" ]; then
    ...
    cat "$CONFIG_FILE"
    read -p "是否使用上述命令启动bench_serving? [y/n] " use_existing
    if [ "$use_existing" = "y" ] || [ "$use_existing" = "Y" ]; then
        # 执行配置文件中的命令
        eval "$(cat "$CONFIG_FILE")"
        echo "bench_serving已启动"
    else
...
# 1. 提示用户输入容器名称和镜像版本
read -p "请输入py文件路径（如：path/*.py）: " path

extra_args=()
echo "请输入额外参数（如-H "Content-Type: application/json"等，输入空值结束）"
while true; do
    read -p "输入参数: " arg
    if [ -z "$arg" ]; then
        break
...
# 构建最终命令
bench_serving_cmd="python3 ${path} ${extra_args[*]}"

# 3. 保存命令到配置文件
echo "$bench_serving_cmd" > "$CONFIG_FILE"
echo "命令已保存至: $CONFIG_FILE"
echo "生成的命令: $bench_serving_cmd"

# 询问是否执行命令
read -p "是否立即执行上述命令? [y/n] " execute_now
if [ "$execute_now" = "y" ] || [ "$execute_now" = "Y" ]; then
    eval "$bench_serving_cmd"
    echo "bench_serving启动完成"
else
    echo "可稍后通过执行配置文件启动: source $CONFIG_FILE"
fi
